  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className={`max-w-4xl max-h-[80vh] ${language === 'ar' ? 'text-right direction-rtl' : 'text-left'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
        <DialogHeader>
          <DialogTitle className={`text-xl font-bold mb-4 ${language === 'ar' ? 'text-right arabic-text' : 'text-left'} text-foreground`}>
            {t('hero.learn_more')}
          </DialogTitle>
        </DialogHeader>
        ...
        <ScrollArea className={`h-[60vh] ${language === 'ar' ? 'pl-4' : 'pr-4'}`}>
          <div className={`space-y-6 ${language === 'ar' ? 'text-right arabic-text' : 'text-left'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <section>
              <h3 className={`text-lg font-bold mb-3 text-primary ${language === 'ar' ? 'arabic-text' : ''}`}>
                {t('about.platform_intro')}
              </h3>
              <p className={`text-sm leading-relaxed text-muted-foreground whitespace-pre-line ${language === 'ar' ? 'arabic-text' : ''}`}>
            ...
            {/* Meaning and Values */}
            <section>
              <h4 className={`text-base font-semibold mb-3 text-foreground ${language === 'ar' ? 'arabic-text' : ''}`}>
                {t('about.meaning_title')}
              </h4>
              <ul className={`space-y-2 ${language === 'ar' ? 'arabic-list' : ''}`} style={language === 'ar' ? { direction: 'rtl', textAlign: 'right' } : {}}>
                <li className={`arabic-list-item text-sm leading-relaxed text-muted-foreground ${language === 'ar' ? 'arabic-text' : ''}`}>
            ...
            {/* Commitment */}
            <section>
              <p className={`text-sm leading-relaxed text-muted-foreground ${language === 'ar' ? 'arabic-text' : ''}`}>
                {t('about.commitment')}
              </p>
            </section>
