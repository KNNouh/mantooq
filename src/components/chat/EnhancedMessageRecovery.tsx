...
export function EnhancedMessageRecovery({
  ...
  useEffect(() => {
    const shouldShow = connectionHealth.status === 'error' || connectionHealth.status === 'degraded' || connectionHealth.status === 'disconnected' && connectionHealth.consecutiveFailures > 0 || connectionHealth.quality < 50;
  ...
  useEffect(() => {
    if (connectionHealth.status === 'error' && connectionHealth.consecutiveFailures >= 3 && autoRecoveryAttempts < 2 && Date.now() - lastRecoveryTime > 30000) {
      // Wait 30s between auto-recoveries
  ...
  const recoverMessages = useCallback(async (isAutoRecovery = false) => {
    ...
    try {
      let query = supabase.from('messages').select('id, role, content, created_at, conversation_id').eq('conversation_id', conversationId).eq('user_id', userId).order('created_at', {
        ascending: true
      });
      ...
      const since = Math.max(Date.now() - lookbackTime, lastMessageId ? Date.now() - 180000 : 0 // 3 minutes if we have lastMessageId
      );
  ...
  const getConnectionIcon = () => {
    switch (connectionHealth.status) {
      case 'connected':
        return connectionHealth.quality > 80 ? <Wifi className="h-4 w-4 text-green-600 dark:text-green-400" /> : <Signal className="h-4 w-4 text-yellow-600 dark:text-yellow-400" />;
      case 'degraded':
  ...
  return <div className={`flex flex-col gap-2 ${className}`}>
      {/* Connection Status */}
      {statusMessage && <div className="flex items-center justify-center p-2">
          
        </div>}

      {/* Recovery Actions */}
      {showRecovery && <div className="flex items-center justify-center p-3">
          <div className="flex items-center gap-3 bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
            ...
            <span className="text-sm text-yellow-800 dark:text-yellow-200">
              {connectionHealth.status === 'error' ? '🚨 Connection failed. Messages may be missing.' : '⚠️ Poor connection detected. Messages might be missing.'}
            </span>
            <div className="flex gap-2">
              <Button onClick={() => recoverMessages()} disabled={isRecovering} size="sm" variant="outline" className="border-yellow-300 text-yellow-800 hover:bg-yellow-100 dark:border-yellow-600 dark:text-yellow-200 dark:hover:bg-yellow-900/20">
                {isRecovering ? <RefreshCw className="h-3 w-3 animate-spin mr-1" /> : <RefreshCw className="h-3 w-3 mr-1" />}
                Recover
              </Button>
              <Button onClick={handleForceRefresh} disabled={isRecovering} size="sm" variant="default" className="bg-yellow-600 hover:bg-yellow-700 text-white">
                Force Refresh
              </Button>
            </div>
          </div>
      ...
      {autoRecoveryAttempts > 0 && <div className="flex items-center justify-center p-1">
